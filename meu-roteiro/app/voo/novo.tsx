...
export default function NovoVooScreen() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      console.log('[voo/novo] payload:', JSON.stringify(payload, null, 2));
      const { data, error } = await supabase.from('flights').insert(payload).select();
      console.log('[voo/novo] data:', JSON.stringify(data));
      console.log('[voo/novo] error:', JSON.stringify(error));
      if (error) throw error;
      router.back();
    } catch (e) {
      const msg = e instanceof Error ? e.message : JSON.stringify(e);
      console.log('[voo/novo] catch:', msg);
    ...
    <>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Pressable style={[styles.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Salvar Voo</Text>}
        </Pressable>
      </ScrollView>
    </>
  );
